...
@Component({
  ...
  template: `<div class="admin-container"><div class="page-header"><h2>{{ 'admin.employees.title' | translate }}</h2><button mat-raised-button color="primary" (click)="openDialog()" *appHasPermission="'admin.employees.manage'"><mat-icon>add</mat-icon>{{ 'admin.employees.addEmployee' | translate }}</button></div><mat-card><mat-card-content><p class="coming-soon">{{ 'admin.employees.comingSoon' | translate }}</p></mat-card-content></mat-card></div>`,
